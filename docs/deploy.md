...
- **`CATALOG_GITHUB_TOKEN`** — on the **`catalog-sync` service only**. Anonymous GitHub reads get 60 requests/hour keyed on the *IP*, and Railway's egress IP is shared, so other tenants can spend that budget before the cron wakes; the 2026-08-01 run died on its first request that way. A token gets 5,000/hour of its own. It needs **no scopes** — the datamine is public — and the sync sends it only to `api.github.com` and `raw.githubusercontent.com`. Prefer a classic token with no expiry: a fine-grained one expires within 366 days, and an expired or revoked token is *worse* than none — GitHub answers `401`, which the sync treats as non-transient and dies on, where anonymous would at least have had a chance. That failure is loud and self-describing (the watchdog issue quotes `Bad credentials`), but it is still a night of staleness on a timer. An absent or malformed token, by contrast, only warns and continues anonymously.

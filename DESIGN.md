...
**The One Amber Rule.** Medal Amber (or its Deep day form) covers at most 10% of any screen, and marks three things: the **primary action** (the commit), **record emphasis** (the feat, including the recency glow on one that landed today), and the **hero kicker** that opens a mode landing, which is brand cadence rather than a state. Never selection, never position, never "you are here". A view carries one amber subject. Supporting amber at a different rank does not make a second (a mode landing sets the hero kicker above the monument's numeral), and one condition repeated across peers is still one subject (a wall of OPEN BOUNTY kickers marks a single thing many times). Two amber elements at the same rank marking different things means one of them is wrong. The premium acquisition wash is exempt — it is a *material*, not the accent doing accent work; the amber **ink** on the same screen still counts.

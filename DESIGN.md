...
- **Flag Edge** (#FFFFFF4D · rgba(255,255,255,.3) night / #0A0C104D · rgba(10,12,16,.3) day): the inset 1px ring on a country flag, and the one neutral edge that is deliberately heavier than Hairline. A flag is a fixed set of official colours, so whichever band meets the pane can be the one that matches it — Japan and Nigeria on a light pane, Germany on a dark one — and a Hairline is not enough to close the shape. An inset shadow, never a border: a border inside a control silently costs it hit area. Nothing but a flag wears this.
...
The identity layer. A small, curated, fixed set of battle scenes, each processed **offline once** into `image.jpg` + `image-depth.png` (depth AI; subject separated from background). Runtime renders one textured quad with a WebGL depth-displacement shader: UV offset by `depth.r × pointer/gyro`, nearer pixels moving more, displacement clamped to a few percent — subtle, Apple-like. Scenes swap per nation (and per branch across modes: ground vs air scenery). Above it sits Night Scrim or Day Veil, then grain, then the glass DOM. Fallbacks are part of the component, not afterthoughts: static image where WebGL is absent; effect disabled (static, veiled image) under `prefers-reduced-motion`; the 2-layer cutout (subject PNG over background, pure CSS transform) as the low-end path.

The stack carries one engine-conditional member: a noise **dither** above grain, painted only where the browser does not dither its own gradients. Every pane tint in this system is a shallow alpha ramp, and an engine that quantises one without dithering renders it as hard horizontal bands; the dither supplies what that engine omits so the same pane reads the same in every browser. It is corrective, never expressive — it carries no colour of its own and is calibrated to disappear. `e2e/firefox-dither.spec.ts` is what keeps it honest.
...
- **Back of house (/admin):** the amber primary marks the act that commits, once per decision surface — a form, a dialog, a queue row — and never selection, navigation, or an abstention. A screen may therefore hold several ambers, but only where each is a separate decision to publish something; two ambers competing for the same decision is the fault this rule exists to prevent. Every other admin control stays in the ghost/grey register. Status ink uses the semantic tokens (verified/warn/danger) with day-safe forms, mirroring the accent's Deep pattern.

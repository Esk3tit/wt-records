...
### Spotlight (Browse)
The best feats inside the active filter set, above the ledger: the three highest-kill Current records across the *whole* filtered set — not the page, and independent of the ledger's sort — in the record wall's own cards under a ruled section label. It is a summary, so it appears only when it has something to summarise: absent unfiltered (the Mode landing already shows the Mode's best), and absent unless the filter set holds enough titles that three of them read as a selection rather than a restatement — a count of *held titles*, never of rows, which open bounties would inflate. No acquisition tint here, unlike the nation sheet's wall — among three ranked cards a lone gilded pane reads as a medal rather than as premium. Below sm it becomes a snap rail rather than a stack, so the ledger stays on screen.

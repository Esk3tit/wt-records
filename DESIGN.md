...
**The One Amber Rule.** Medal Amber (or its Deep day form) covers at most 10% of any screen, and marks two things only: the **primary action** and **record emphasis** — the commit and the feat. Never selection, never position, never "you are here". Two competing amber elements in one view means one of them is wrong. The premium acquisition wash is exempt — it is a *material*, not the accent doing accent work; the amber **ink** on the same screen still counts.

**The Rationed Colour Rule.** A colour whose meaning comes from *scarcity* cannot live where recurrence is guaranteed. Persistent chrome — the nav, and anything else a reader meets on every page and cannot dismiss — is guaranteed recurrence, so the rationed colours are barred from it: Medal Amber and the rank metals. A surface seen once and dismissed for good is not chrome, which is why the consent banner still commits in the amber primary. A colour whose meaning is *state* is the opposite case, and belongs in chrome, because chrome is where state is shown. This is the reason behind the two rules above, not a third restriction beside them: a ceiling on amber only holds if amber is absent from the surfaces that repeat. The wordmark's amber separator is exempt on the grounds `e2e/nav-contrast.spec.ts` already uses — it is a mark, carrying no reading.
...
### Navigation
- **Mode switcher is the primary nav:** GRB/GAB/ARB/AAB as text links in the floating glass header (thin material when parked) — the active mode in the neutral **pill-active** fill at full ink, inactive in muted ink; 4px-radius hover surface. **Pill-active is the site's one selected-state vocabulary**, not a nav token: the mode switcher, the catalog filters, the ledger's pagination, the admin combobox, the vehicle lookup and the claim panel all say "this one is chosen" in the same fill, and a surface that means selection anywhere means it everywhere. The accent is deliberately not spent here — see The Rationed Colour Rule; the switcher sits in chrome, so amber in it would be amber on every page. Nav links are not underlined (chrome opts out); content links keep underlines with 2px offset as a non-color affordance.

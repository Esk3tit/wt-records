...
1. **Wrap the app in `PreviewProvider`** (a bundle export). Several components (`LeaderboardList`, `RecordMonument`, `TopRecords`, `LatestRecord`, `RecordName`, `SiteNav`) render internal links and **crash with a router-context error without it**:

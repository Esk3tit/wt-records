...
**Claim request**:
A pending Claim — not a separate entity. Filed by a logged-in User; the link exists only once a moderator approves. Claims are never self-serve (impersonation of known holders is one click otherwise). A denied request is **remembered** on the claim row — it leaves no trace on the Player, but that User can never ask for that Player again unless a moderator clears the denial. The denying moderator may record a reason, which is what a later moderator weighs a clear against; optional, unlike a Revoke's, because a queue decision usually speaks for itself. Requesting while already holding a Claim is refused with a plain error: unlike an Amendment, a claim request is not shadowed.

**Revoke** (a Claim):
The only way out of a Claim, and a Moderator act — one action serving three situations (a moderator's own mistake, a holder asking to leave, an abuse), told apart afterwards by the **reason** the moderator records with it. The Player returns to the accountless state and Records and Snapshots never move. It frees the Player, not the User: a revoked User may request that same Player again, or any other — a **denial**, not a revoke, is what makes a refusal permanent. Approving, denying, clearing a denial and revoking all land in the audit log against the Player.
